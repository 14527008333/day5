...
$(function() {
	$.get("<%=request.getContextPath()%>/duoxuan.do",function(hh){
		for(var i in hh){
			var h=$('<input class="xx" type="checkbox" value="'+hh[i].fid+'" name="fids"><span>'+hh[i].fname+'</span>')
			$(".whh").append(h)
		}
	})
	var did=$("[name=did]").val()
	$.get("<%=request.getContextPath()%>/huixian.do?did="+did,function(www){
		$("[name=dname]").val(www.dname)
		$("[name=jqjs]").val(www.jqjs)
		$("[name=writer]").val(www.writer)
		$("[name=fxdate]").val(www.fxdate)
		var w=www.ffname
		var ww=w.split(",");
		for(var i in ww){
			$(".xx[value="+ww[i]+"]").attr("checked",true)
		}
	})
})	
